 ...
 //num len = s.size();
  //ll n,k,ans=0,flag=0,temp=0,f1=0,count=0,f2=0,sum=0,xx=0,minn1=big,minn2=big,maxx1=small,maxx2=small;
	tc
	{
     ...
     for (ll i = 0; i < n; i++) {
       /* code */
       char char1=s[max(0,i-1)];
        char char2=s[min(n-1,i+1)];
        if (s[i]!=char1) {
          /* code */
          v[s[i]-'a'].push_back(char1);
        }
        if (s[i]!=char2) {
          /* code */
          v[s[i]-'a'].push_back(char2);
        }
