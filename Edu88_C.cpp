...
int main()
{
  ...
  tc
  {
     ...
     ll h,c,t;
     cin>>h>>c>>t;
     // only changes in avg temp on odd iteration
     // let for some iteration x abs.. integer
     // t(nearly possibe) = ( h*x + (x-1)*c )/(2*x-1)
     // x= (t-c)/(2*t-h-c)
     // you have to check for x+1 iteration also for smaller diffrence
     if (t==h) {
       /* code */
       p1(1)
     }
     ...
     else
     {
       ll x= (t-c)/(2*t-h-c);
       ll y=x+1;
       ld ans1=abs(t- ( h*x + (x-1)*c )/(1.0*(2*x-1)));
       ld ans2=abs(t- ( h*y + (y-1)*c )/(1.0*(2*y-1)));
       if (ans1<=ans2) {
         /* code */
         p1(2*x-1)
       }
