...
//  clock_t launch=clock();
 ...
 //num len = s.size();
  ...
  tc
  {
     ll n=0,k=0,m=0,flag=0,temp=0,f1=0,count=0,f2=0,sum=0,xx=0,minn1=big,minn2=big,maxx1=small,maxx2=small;
     cin>>n;
     double dd=2*n;
     double ans=0.0;
     double angle=((dd-2)*180)/dd;
     double start_angle=angle-90;
     //trace(angle)
     lp(i,0,n/2-1)
     {
      // trace(start_angle)
       ans+=sin(start_angle*pi/180);
       double xx=90-start_angle;
       xx+=90;
      // trace(xx)
       start_angle=angle-xx;
      // trace(ans)
