...
//  clock_t launch=clock();
 ...
 //num len = s.size();
  ll n,m,k,ans=0,flag=0,temp=0,f1=0,count=0,f2=0,sum=0,xx=0,minn1=big,minn2=big,maxx1=small,maxx2=small;
  cin>>n>>m;
  ll mat[n][3];
  ans=big;
  ...
  lp (i,0,n) {
    ...
    lp (j,0,m) {
      /* code */
      ll temp =min(j,m-j); //circular
      if (s[j] == '#' || s[j] == '*' || s[j] == '&')
      {
         mat[i][0] = min (mat[i][0], temp);
      }
      if (isalpha(s[j])) {
        mat[i][1] = min(mat[i][1], temp);
      }
      if (isdigit(s[j])) {
        mat[i][2] = min(mat[i][2], temp);
      }
    ...
    lp(j,0,n)
    {
      if(i!=j)
      {
        lp(k,0,n)
        {
          if (i!=k && j!=k) {
            /* code */
            ans=min(ans,mat[i][0]+mat[j][1]+mat[k][2]);
          }
        }
      }
    }
